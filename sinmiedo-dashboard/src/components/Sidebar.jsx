...
import { fmt, plColor, clearProfile, clearEntries, clearAccounts, greeting } from '../utils'
...
export default function Sidebar({ activePage, onNavigate, monthTotal, pct, profile, onSignOut }) {
  const [showProfileMenu, setShowProfileMenu] = useState(false)
  const color = plColor(monthTotal)
  const greet = greeting()

  function handleSignOut() {
    if (!window.confirm('Sign out? Your data will remain saved on this device.')) return
    clearProfile()
    onSignOut()
  }
  ...
  function handleClearData() {
    ...
    clearEntries(profile?.name)
    clearAccounts(profile?.name)
    clearProfile()
    onSignOut()
  }
    ...
    <>
      ...
      <aside className="hidden sm:flex flex-col w-[220px] flex-shrink-0 border-r border-border bg-[#080c14]">
        ...
        <div className="p-5 border-b border-border">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2.5">
              ...
              <div className="min-w-0">
                <p className="text-xs font-bold text-txt tracking-tight leading-none truncate">Sin Miedo Capital</p>
            ...
            <button
              ...
              className="w-7 h-7 rounded-lg bg-surf2 border border-border flex items-center justify-center text-muted hover:text-txt hover:border-green/30 transition-colors flex-shrink-0"
              title="Profile"
            >
              <svg width="13" height="13" viewBox="0 0 13 13" fill="none" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round">
                <circle cx="6.5" cy="4.5" r="2" />
                <path d="M2 11.5c0-2.5 2-4 4.5-4s4.5 1.5 4.5 4" />
              </svg>
            </button>
          </div>
          ...
          {profile && (
            <div className="mb-4">
              <p className="text-sm font-semibold text-txt">{greet}, {profile.name}! 👋</p>
              <p className="text-[10px] text-muted mt-0.5">Welcome back to your dashboard</p>
            </div>
          )}

          {/* Profile dropdown */}
          {showProfileMenu && (
            <div className="mb-4 card border-border/80 p-3 space-y-2 bg-surf2">
              <p className="text-[10px] text-muted uppercase tracking-wider font-semibold">Profile</p>
              {profile && (
                <p className="text-xs text-txt font-medium">{profile.name}</p>
              )}
              <button
                onClick={handleSignOut}
                className="w-full text-left text-xs text-muted hover:text-txt py-1 transition-colors"
              >
                Sign out
              </button>
              <button
                onClick={handleClearData}
                className="w-full text-left text-xs text-red hover:text-red/80 py-1 transition-colors"
              >
                Clear all my data
              </button>
            </div>
...
function MonthIcon({ size = 16 }) {
  return (
    <svg width={size} height={size} viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round">
      ...
      <path d="M8 8 L8 3M8 8 L11.5 10" />
    </svg>
  )
}

function AccountIcon({ size = 16 }) {
  return (
    <svg width={size} height={size} viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round">
      <rect x="2" y="5" width="12" height="8" rx="1.5" />
      <path d="M5 5V4a3 3 0 0 1 6 0v1" />
      <path d="M8 10v-1.5M6.5 10h3" />
    </svg>
  )
}
